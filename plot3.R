...
plot(df$Sub_metering_1 ~ df$dateTime, type = "l", ylab = "Energy sub metering", xlab = "") #create plot area
lines(df$Sub_metering_2 ~ df$dateTime, col = "red") #add submeter2 red lines
lines(df$Sub_metering_3 ~ df$dateTime, col = "blue") #add submeter3 blue lines
legend("topright", col = c("black", "red", "blue"), c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), lty = c(1,1), lwd = c(1,1)) #create legend
